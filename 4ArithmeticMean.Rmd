...
modelRF <- randomForest(SalePrice ~ ., data=train)

modelSVM <- svm(SalePrice ~ ., data=train)

prediction <- (
  predict(modelLM,  test, type="response") +
  predict(modelRF,  test, type="response") +
...
[Hmmm...](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/leaderboard)
...
modelLM <- stfu(stepAIC(lm(SalePrice ~ ., data=temp)))
modelRF <- randomForest(SalePrice ~ ., data=temp)
modelSVM <- svm(SalePrice ~ ., data=temp)

prediction <- (
  predict(modelLM,  temp2, type="response") +
  predict(modelRF,  temp2, type="response") +
  predict(modelSVM, temp2, type="response")
) / 3

cbind(temp2, prediction) %>%
  dplyr::transmute(Id, SalePrice = prediction) %>%
  write_csv("predictions/prediction4.csv")
```
