...
```{r lm}
model1 <- glmnet(SalePrice ~ ., data=train)

prediction <- as.data.frame(predict(model1, test, type="response"))

model_output <- cbind(test, prediction=prediction$s84) %>%
  mutate(
...
[Disappointing.](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/leaderboard)
...
model1 <- glmnet(SalePrice ~ ., data = temp)
prediction <- as.data.frame(predict(model1, temp2, type="response"))

cbind(temp2, prediction=prediction$s84) %>%
  dplyr::transmute(Id, SalePrice = prediction) %>%
  write_csv("predictions/predictionElasticnet.csv")
```
